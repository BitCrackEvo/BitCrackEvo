...
__constant__ unsigned int _START_EXPONENT[8];
__constant__ unsigned int _STRIDE[8];
...
__global__ void multiplyStepKernel(int pointsPerThread, int step, unsigned int *chain, const unsigned int *gxPtr, const unsigned int *gyPtr);
...
cudaError_t setStride(const secp256k1::uint256 &stride)
{
    unsigned int strideWords[8];
 
    stride.exportWords(strideWords, 8, secp256k1::uint256::BigEndian);
 
    cudaError_t err = cudaMemcpyToSymbol(_STRIDE, strideWords, sizeof(unsigned int) * 8);
 
    return err;
}

cudaError_t setStartExponant(const secp256k1::uint256 &startExponant)
{
    unsigned int startExponantWords[8];
 
    startExponant.exportWords(startExponantWords, 8, secp256k1::uint256::BigEndian);
 
    cudaError_t err = cudaMemcpyToSymbol(_START_EXPONENT, startExponantWords, sizeof(unsigned int) * 8);
 
    return err;
}

cudaError_t CudaDeviceKeys::init(int blocks, int threads, int pointsPerThread, const secp256k1::uint256 &privateKey, const secp256k1::uint256 &stride)
{
  _blocks = blocks;
  _threads = threads;
  _pointsPerThread = pointsPerThread;
  _totalPoints = (unsigned long) _pointsPerThread * _threads * _blocks;

  // Allocate space for public keys on device
  cudaError_t err = initializePublicKeys(_totalPoints);
  ...
  // Allocate start private key on device
  err = setStartExponant(privateKey);
  if(err) {
	return err;
  }

  // Allocate stride between private keys on device
  err = setStride(stride);
  if(err) {
	return err;
  }
...
{
  multiplyStepKernel <<<_blocks, _threads>>>(_pointsPerThread, _step, _devChain, _devBasePointX, _devBasePointY);
...
__device__ static void calcPrivateKeys(const unsigned int start[8], const unsigned int stride[8], const int idx, unsigned int x[8])
{
...
__global__ void multiplyStepKernel(int pointsPerThread, int step, unsigned int *chain, const unsigned int *gxPtr, const unsigned int *gyPtr)
{
  ...
  for(int i = 0; i < pointsPerThread; i++) {
	...
	unsigned int p[8];
    calcPrivateKeys(_START_EXPONENT, _STRIDE, i, p);
...
bool CudaDeviceKeys::selfTest(const secp256k1::uint256 &startExponent, const secp256k1::uint256 &stride)
{
  ...
  for(int block = 0; block < _blocks; block++) {
	for(int thread = 0; thread < _threads; thread++) {
	  for(int idx = 0; idx < _pointsPerThread; idx++) {
        ...
        secp256k1::uint256 index = secp256k1::uint256(getIndex(block, thread, idx));
        secp256k1::uint256 indexWithStride = index * stride;

		secp256k1::uint256 privateKey = indexWithStride + startExponent;
